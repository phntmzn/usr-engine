...
def chord_pattern(index: int) -> Path:
    ...
    root_indices = torch.randint(0, len(chromatic_scale), (TOTAL_FILES,), device=device)
    ...
    step_tensor = torch.arange(total_steps, device=device)
    bar_index_tensor = step_tensor // steps_per_bar
    prog_idx_tensor = bar_index_tensor % len(progression_degrees)
    scale_degrees = torch.tensor([progression_degrees[i] for i in prog_idx_tensor.tolist()], device=device)
